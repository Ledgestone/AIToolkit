...
class AITool(ABC):
    ...
    def process(self) -> "AITool":
        if not self.can_process():
            raise AINonRetryableError(f"Cannot process {self} because not all inputs are present.\n \
    ...
    def why_cannot_process(self) -> str:
        ...
        required_inputs = self.required_input + self.dynamic_input
        missing_inputs = [
            key for key in required_inputs if key not in self.input]
        if len(missing_inputs) > 0:
            return f"{self} cannot process because the following inputs are missing: {missing_inputs}"

        for input in self.input.values():
            if isinstance(input, AITool) and not input._has_single_output(self):
                return f"{self} cannot process because {input} does not have a single output."

        return f"{self} cannot process for an unknown reason."

    def _get_single_output(self, getter: "AITool") -> Any:
        if not self._has_single_output(getter):
            raise Exception(
                f"Cannot get single output from {self} for {getter} because there are no outputs ready.")
