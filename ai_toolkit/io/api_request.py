...
class APIRequest(AITool):
    def __init__(self, name):
        ...
        self.optional_input = ["headers", "body",
                               "params", "auth", "json", "timeout"]

    def _process(self) -> Any:
        url = self._get_from_input("url")
        method = self._get_from_input("method")
        headers = self._get_from_input(
            "headers") if "headers" in self.input else None
        body = self._get_from_input("body") if "body" in self.input else None
        params = self._get_from_input(
            "params") if "params" in self.input else None
        ...
        timeout = self._get_from_input(
            "timeout") if "timeout" in self.input else None
        ...
        try:
            if method == "GET":
                ...
                response = requests.post(
                    url, headers=headers, data=body, params=params, auth=auth, json=json, timeout=timeout)
            elif method == "PATCH":
