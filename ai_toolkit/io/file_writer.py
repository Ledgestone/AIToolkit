...
class FileWriter(AITool):
    ...
    def _process(self) -> str:
        ...
        try:
            ...
            raise AINonRetryableError(f"File path '{file_path}' does not contain a file type")

        # Check that the file type is valid
        if file_type not in ["txt", "json"]:
            raise AINonRetryableError(f"File type '{file_type}' is not a valid file type, must be 'txt' or 'json'")
        ...
        try:
            ...
            raise AINonRetryableError(f"File '{file_path}' could not be encoded with encoding '{file_encoding}'")
        except Exception as e:
            raise AINonRetryableError(f"Unknown error occurred when writing to file '{file_path}': {e}")
